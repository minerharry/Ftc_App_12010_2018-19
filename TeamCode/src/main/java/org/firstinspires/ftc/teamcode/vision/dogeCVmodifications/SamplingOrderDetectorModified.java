...
public class SamplingOrderDetectorModified extends DogeCVDetector {
    ...
    //Create the roundness scorer for the silver
    ...
    public SamplingOrderDetectorModified() {
        super();
    ...
    public Mat process(Mat input) {
        ...
        for(MatOfPoint c : contoursWhite){
            ...
            double diffrenceScore = calculateSilverScore(points);
        ...
        if(choosenWhiteRect.get(0) != null && choosenWhiteRect.get(1) != null  && chosenYellowRect != null){
            ...
            if(leftCount == 0){
                currentOrder = SamplingOrderDetectorModified.GoldLocation.LEFT;
            }

            if(leftCount == 1){
                currentOrder = SamplingOrderDetectorModified.GoldLocation.CENTER;
            }

            if(leftCount >= 2){
                currentOrder = SamplingOrderDetectorModified.GoldLocation.RIGHT;
            }
            isFound = true;
            lastOrder = currentOrder;

        }else{
            currentOrder = SamplingOrderDetectorModified.GoldLocation.UNKNOWN;
            isFound = false;
        }
